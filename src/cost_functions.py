...
def get_cost(depot_index,raf_index,biomass_mat,dist_mat,pellet_mat,forecast,adjust,ignore_constraints=False):
  ...
  total_cost=np.sum(underutil_pellet)+np.sum(underutil_raf)+np.sum(transport_biomass)*0.001+np.sum(transport_pellet)*0.001+np.sum(np.abs(adjust))
  return np.sum(underutil_pellet),np.sum(underutil_raf),np.sum(transport_biomass)*0.001,np.sum(transport_pellet)*0.001,total_cost
...
def create_submission_file(forecast_2018,forecast_2019,depot_index,raf_index,biomass_mat_2018,biomass_mat_2019,pellet_mat_2018,pellet_mat_2019):
    ...
    df_biomass_2018=pd.melt(pd.DataFrame(biomass_mat_2018,columns=depot_index).reset_index(),id_vars="index").rename(columns={"index": "source_index", "variable": "destination_index"})
    df_biomass_2018["data_type"]="biomass_demand_supply"
    df_biomass_2018["year"]="2018"

    #biomass demand_supply 2018
    df_biomass_2019=pd.melt(pd.DataFrame(biomass_mat_2019,columns=depot_index).reset_index(),id_vars="index").rename(columns={"index": "source_index", "variable": "destination_index"})
    df_biomass_2019["data_type"]="biomass_demand_supply"
    df_biomass_2019["year"]="2019"

    #pellet demand_supply 2018
    df_pellet_2018=pd.melt(pd.DataFrame(pellet_mat_2018,index=depot_index,columns=raf_index).reset_index(),id_vars="index").rename(columns={"index": "source_index", "variable": "destination_index"})
    df_pellet_2018["data_type"]="pellet_demand_supply"
    df_pellet_2018["year"]="2018"

    #pellet demand_supply 2019
    df_pellet_2019=pd.melt(pd.DataFrame(pellet_mat_2019,index=depot_index,columns=raf_index).reset_index(),id_vars="index").rename(columns={"index": "source_index", "variable": "destination_index"})
    df_pellet_2019["data_type"]="pellet_demand_supply"
    df_pellet_2019["year"]="2019"

    df=pd.concat([df,df_depot_loc,df_refinery_loc,df_forecast_2018,df_biomass_2018,df_pellet_2018,df_forecast_2019,df_biomass_2019,df_pellet_2019])
